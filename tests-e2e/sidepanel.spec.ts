...
test('loads the unpacked extension side panel without layout overflow', async () => {
  ...
  try {
    ...
    await expect(page.locator('#pinterest-board-id')).toBeVisible()
    await expect(page.locator('#create-board')).toBeVisible()
    await expect(page.getByRole('button', { name: 'Fetch Models' })).toBeVisible()
    await page.locator('#product-source').selectOption('amazon')
    await expect(page.locator('#amazon-settings')).toBeVisible()
    await expect(page.locator('#shopee-settings')).toBeHidden()
    await expect(page.locator('#amazon-image-state')).toContainText('creates an original typographic poster automatically')
    await expect(page.getByRole('button', { name: 'Test Amazon product lookup' })).toBeVisible()
    await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth <= window.innerWidth)).toBe(true)
    expect(consoleErrors).toEqual([])
