...
export function extractShopeeCandidates(root: ParentNode): ProductCandidate[] {
  const explicitCards = Array.from(root.querySelectorAll<HTMLElement>(
    '[data-product-id], [data-testid*="product-card" i], [class*="product-card" i], .product-offer-item, [class*="AffiliateItemCard"]',
  ))
  const inferredCards = Array.from(root.querySelectorAll<HTMLAnchorElement>('a[href]'))
    ...
    .map((anchor) => anchor.closest<HTMLElement>('article, li, [role="listitem"], [class*="card" i]') ?? anchor)
  const cards = [...new Set([...explicitCards, ...inferredCards])]
  ...
  return cards.flatMap((card) => {
    ...
    const ratingMatch = text.match(/(?:rating|bintang)\s*([0-5](?:[.,]\d+)?)/i)
      ?? text.match(/\b([4-5][.,]\d)\b/)
    const soldMatch = text.match(/([\d.,]+\s*(?:RB|K|JT|M)?)\s*(?:terjual|sold)/i)
    ...
    return [{
      ...
      rating: ratingMatch ? Number.parseFloat(ratingMatch[1].replace(',', '.')) : 0,
      sold: soldMatch ? parseIndonesianCount(soldMatch[1]) : 0,
