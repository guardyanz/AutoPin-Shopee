...
export function rankProducts(products: ProductCandidate[]): ProductCandidate[] {
  ...
  const soldValues = eligible.map((product) => Math.log10(product.sold + 1))
  const commissionValues = eligible.map((product) => product.commissionPercent)
  const ratingValues = eligible.map((product) => product.rating)
  ...
  const score = (product: ProductCandidate): number => (
    ...
    + normalize(Math.log10(product.sold + 1), ...ranges.sold) * 0.35
    + normalize(product.rating, ...ranges.rating) * 0.25
  )
